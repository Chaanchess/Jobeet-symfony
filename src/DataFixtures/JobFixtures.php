...
class JobFixtures extends Fixture implements DependentFixtureInterface
{
    ...
    public function load(ObjectManager $manager) : void
    {
        ...
        $jobExpired->setCompany('Sensio Labs expired prueba');
        $jobExpired->setLogo('sensio-labs.gif');
        $jobExpired->setUrl('http://www.sensiolabs.com/');
        $jobExpired->setPosition('Web Developer');
        $jobExpired->setLocation('Madrid, Españita');
        $jobExpired->setDescription('You\'ve already developed websites with symfony and you want to work with Open-Source technologies. You have a minimum of 3 years experience in web development with PHP or Java and you wish to participate to development of Web 2.0 sites using the best frameworks available.');
        $jobExpired->setHowToApply('Send your resume to fabien.potencier [at] sensio.com');
        $jobExpired->setPublic(true);
        $jobExpired->setActivated(true);
        $jobExpired->setToken('job_sensio_labsExpired');
        $jobExpired->setEmail('job@example.com');
        $jobExpired->setExpiresAt(new \DateTime('-30 days'));

        for($i = 30; $i<60;$i++){
            $job = new Job();
            $job->setCategory($manager->merge($this->getReference('category-programming')));
            $job->setType('full-time');
            $job->setCompany('Category programming '.$i);
            $job->setLogo('sensio-labs.gif');
            $job->setUrl('http://www.sensiolabs.com/');
            $job->setPosition('Web Developer');
            $job->setLocation('Berlin, Alemania');
        ...
        for($i = 61; $i<90;$i++){
            $jobDesign = new Job();
            $jobDesign->setCategory($manager->merge($this->getReference('category-design')));
            $jobDesign->setType('full-time');
            $jobDesign->setCompany('Category design '.$i);
            $jobDesign->setLogo('extreme-sensio.gif');
            $jobDesign->setUrl('http://www.extreme-sensio.com/');
            $jobDesign->setPosition('Web Designer');
            $jobDesign->setLocation('Madrid, España');
            $jobDesign->setDescription('You\'ve already developed websites with symfony and you want to work with Open-Source technologies. You have a minimum of 3 years experience in web development with PHP or Java and you wish to participate to development of Web 2.0 sites using the best frameworks available.');
            $jobDesign->setHowToApply('Send your resume to fabien.potencier [at] sensio.com');
            $jobDesign->setPublic(true);
            $jobDesign->setActivated(true);
            $jobDesign->setToken('job_'.$i);
            $jobDesign->setEmail('job@example.com');
            $jobDesign->setExpiresAt(new \DateTime('+30 days'));
            $manager->persist($jobDesign);
        }
